...
class NmapMgmt:
    ...
    def create(self,targets,options):
        ...
        if self.__check_options(options):
            taskID = NmapProcess(targets,options)#TODO
        else:
            taskID = "INVALID"
    ...
    def execute(self,targets,options):
        ...
        try:
            ...
            task_id = create_res.get('retValue')#TODO   ?
            taskID = NmapAgentAPI.task_table.get(task_id)
            if taskID is None:
                return commons.RetInfo(0x0101).Info
            elif taskID == "INVALID":
    ...
    def get_tool_status(self,component="Nmap"):#TODO
        """Method for getting tool status,such as version,cpu etc.
    ...
    def __check_options(self,options):# TODO
